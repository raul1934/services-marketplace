...
export interface WizFooter {
  ...
  slide?: { label: string; doneLabel: string; onConfirm: () => void; disabled?: boolean; confirmHint?: string; resetSignal?: number };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: t.colors.bg }} edges={['top', 'bottom']}>
     ...
     <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center', paddingHorizontal: 20, paddingTop: 13, paddingBottom: 26, borderTopWidth: 1, borderColor: t.colors.line, backgroundColor: t.colors.surface }}>
        {footer.slide ? (
          <View style={{ flex: 1 }}>
            <SlideToConfirm label={footer.slide.label} doneLabel={footer.slide.doneLabel} disabled={footer.slide.disabled} onConfirm={footer.slide.onConfirm} confirmHint={footer.slide.confirmHint} resetSignal={footer.slide.resetSignal} />
          </View>
        ) : footer.primary ? (
          <>
            {footer.back && (
