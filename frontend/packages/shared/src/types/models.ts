...
export interface ServiceRequest {
  ...
  /** TEMP (test bots): request was generated by a bot. Remove with backend app/Bots. */
  is_test?: boolean;
  /**
   * TEMP (test bots): a bot client has nobody to read the start_code aloud, so
   * the accepted provider gets it directly on test jobs only. Remove with app/Bots.
   */
  test_start_code?: string | null;
  ...
  /** Server-decided pre-arrival actions (owner-only). See ServiceRequest::canReschedule. */
  can_reschedule?: boolean;
  can_report_no_show?: boolean;
  /** When the provider's promised arrival window closes; unlocks the two above. */
...
export interface WarrantyClaim {
  ...
  /** Evidence the client attached when opening the claim. */
...
export interface Proposal {
  ...
  status: ProposalStatus;
  /** TEMP (test bots): bid placed by a bot provider. Remove with backend app/Bots. */
  is_test?: boolean;
  created_at?: string;
