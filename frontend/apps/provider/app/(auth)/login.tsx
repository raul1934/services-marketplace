...
export default function Login() {
  ...
  const [email, setEmail] = useState(__DEV__ ? 'prestador@chamafacil.test' : '');
  ...
  return (
    <Screen scroll={false} padded={false}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 26, paddingTop: 8, paddingBottom: 26, gap: 13 }}>
        ...
        {mode === 'phone' ? (
          <AuthField icon="phone" label={tr('login.phone')} prefix="+55" value={phone} onChangeText={(v) => setPhone(maskPhoneBR(v))} keyboardType="phone-pad" placeholder={tr('login.phonePlaceholder')} error={errors.phone} />
        ) : (
          <>
            <AuthField icon="mail" label={tr('login.email')} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" placeholder={tr('login.emailPlaceholder')} error={errors.email} />
            <AuthField icon="key" label={tr('login.password')} value={password} onChangeText={setPassword} secureTextEntry revealLabel={tr('common.showPassword')} hideLabel={tr('common.hidePassword')} placeholder={tr('login.passwordPlaceholder')} error={errors.password} />
          </>
        )}
