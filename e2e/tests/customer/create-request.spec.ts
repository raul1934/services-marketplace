...
test.describe('Customer · create-request wizard (C07/C08)', () => {
  test('C07 walks all 6 steps and ends on a read-only review with payment', async ({ page }) => {
    await open(page, '/request/new?categoryId=1');
    await expect(page.getByText(/1\s*\/\s*6/i)).toBeVisible();

    // custom question for Guincho
    ...
    await expect(page.getByText(/2\s*\/\s*6/i)).toBeVisible();
    await tap(page, /^continue$/i);

    await expect(page.getByText(/3\s*\/\s*6/i)).toBeVisible();
    await tap(page, /use my location|usar minha/i);
    await page.waitForTimeout(2500);
    await tap(page, /^continue$/i);

    await expect(page.getByText(/4\s*\/\s*6/i)).toBeVisible();
    await tap(page, /^continue$/i);

    await expect(page.getByText(/5\s*\/\s*6/i)).toBeVisible();
